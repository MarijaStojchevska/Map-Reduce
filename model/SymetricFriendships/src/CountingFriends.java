		...
		public void map(LongWritable key, Text input, Context context) throws IOException, InterruptedException 
		   {
			 ...
			 String relationPart1 = relations.split(",")[0];
			 String firstName = relationPart1.replace("[", "").replace("\"", "").trim();
			 String relationPart2 = relations.split(",")[1];
			 String secondName = relationPart2.replace("]", "").replace("\"", "").trim();
			 context.write(new Text(firstName), one);
			 context.write(new Text(secondName), one);
		   }
	} 
